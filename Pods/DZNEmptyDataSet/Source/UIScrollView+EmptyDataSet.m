...
- (CGPoint)dzn_offset
{
    CGFloat top = roundf(self.contentInset.top / 2.0) - 60;
    ...
    if ([self dzn_shouldDisplay] && [self dzn_itemsCount] == 0)
    {
        ...
        if (customView) {
            ...
            if (buttonImage) {
                ...
                // 设置按钮的边框颜色
                [view.button.layer setMasksToBounds:YES];
                [view.button.layer setCornerRadius:15]; //设置矩圆角半径
                [view.button.layer setBorderWidth:1.5];   //边框宽度
                CGColorSpaceRef colorSpace = CGColorSpaceCreateDeviceRGB();
                CGColorRef colorref = CGColorCreate(colorSpace,(CGFloat[]){ 0, 0.396, 0.682, 1 });
                [view.button.layer setBorderColor:colorref];//边框颜色
